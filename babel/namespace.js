...
export function buildTypeNamespace(typeAlias, properties, exported, generatedTypeNames = []) {
    ...
    const literalMembers = properties.map((prop) => {
        ...
        const propSignature = t.tsPropertySignature(key, t.tsTypeAnnotation(typeAnnotation));
    ...
    const typeDecl = t.tsInterfaceDeclaration(typeId, typeAlias.typeParameters ? t.cloneNode(typeAlias.typeParameters) : null, null, t.tsInterfaceBody(literalClone.members));
    const exportedTypeDecl = t.exportNamedDeclaration(typeDecl, []);
    exportedTypeDecl.exportKind = 'type';
    const typeUnionDecl = t.tsTypeAliasDeclaration(t.identifier('Value'), null, t.tsUnionType([
        t.tsTypeReference(t.identifier(typeAlias.id.name)),
        t.tsTypeReference(t.tsQualifiedName(t.identifier(typeAlias.id.name), t.identifier('Data'))),
    ]));
    ...
    const aliasDecls = generatedTypeNames
        ...
        const importEquals = t.tsImportEqualsDeclaration(t.identifier(aliasName), t.identifier(name));
