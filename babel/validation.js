...
import { isArrayBufferReference, isImmutableArrayBufferReference, isBrandReference, isDateReference, isImmutableDateReference, isMapReference, isImmutableUrlReference, isPrimitiveKeyword, isPrimitiveLiteral, isSetReference, isUrlReference, resolveQualifiedRoot, } from './type-guards.js';
...
export function assertSupportedSetType(typePath, declaredTypeNames) {
    ...
    if (typeParams?.params.length !== 1) {
        throw typePath.buildCodeFrameError('Propane Set types must specify a single element type, '
            + 'e.g. Set<string>.');
    }
    const elementPath = typePath.get('typeParameters').get('params')[0];
    assertSupportedType(elementPath, declaredTypeNames);
}
export function assertSupportedMapKeyType(typePath, declaredTypeNames) {
    if (!typePath?.node) {
    ...
    if (typePath.isTSTypeLiteral()) {
        ...
        for (const memberPath of typePath.get('members')) {
            if (!memberPath.isTSPropertySignature()) {
                throw memberPath.buildCodeFrameError('Propane map key object types can only contain property signatures.');
            }
            const keyPath = memberPath.get('key');
            if (!keyPath.isIdentifier() || memberPath.node.computed) {
                throw memberPath.buildCodeFrameError('Propane map key object properties must use simple identifier names.');
            }
            const nestedTypeAnnotation = memberPath.get('typeAnnotation');
            if (!nestedTypeAnnotation?.node) {
                throw memberPath.buildCodeFrameError('Propane map key object properties must include type annotations.');
            }
...
export function assertSupportedTopLevelType(typePath) {
    ...
    throw typePath.buildCodeFrameError('Propane files must export an object type or a primitive-like '
...
export function assertSupportedType(typePath, declaredTypeNames) {
    ...
    if (typePath.isTSTypeReference()) {
        ...
        throw typePath.buildCodeFrameError('Propane property references must refer to imported or locally '
    ...
    if (typePath.isTSTypeLiteral()) {
        for (const memberPath of typePath.get('members')) {
            if (!memberPath.isTSPropertySignature()) {
                throw memberPath.buildCodeFrameError('Propane nested object types can only contain property '
                    + 'signatures.');
            }
            const keyPath = memberPath.get('key');
            if (!keyPath.isIdentifier() || memberPath.node.computed) {
                throw memberPath.buildCodeFrameError('Propane nested object properties must use simple identifier '
                    + 'names.');
            }
            const nestedTypeAnnotation = memberPath.get('typeAnnotation');
            if (!nestedTypeAnnotation?.node) {
                throw memberPath.buildCodeFrameError('Propane nested object properties must include type '
                    + 'annotations.');
            }
            assertSupportedType(nestedTypeAnnotation.get('typeAnnotation'), declaredTypeNames);
        }
    ...
    throw typePath.buildCodeFrameError('Unsupported type in propane file. Only primitives, identifiers, '
