...
import { isArrayBufferReference, isImmutableArrayBufferReference, isBrandReference, isDateReference, isImmutableDateReference, isMapReference, isImmutableUrlReference, isPrimitiveKeyword, isPrimitiveLiteral, isSetReference, isUrlReference, resolveQualifiedRoot, } from './propane-type-guards.js';
export function assertSupportedMapType(typePath, declaredTypeNames) {
    const typeParametersPath = typePath.get('typeParameters');
    if (!typeParametersPath
        || !typeParametersPath.node
        || typeParametersPath.node.params.length !== 2) {
        throw typePath.buildCodeFrameError('Propane Map types must specify both key and value types.');
    }
...
export function assertSupportedSetType(typePath, declaredTypeNames) {
    ...
    if (!typeParams || typeParams.params.length !== 1) {
        throw typePath.buildCodeFrameError('Propane Set types must specify a single element type, e.g. Set<string>.');
    }
    const elementPath = typePath.get('typeParameters').get('params')[0];
    assertSupportedType(elementPath, declaredTypeNames);
}
export function assertSupportedMapKeyType(typePath) {
    ...
    if (typePath.isTSTypeLiteral()) {
        throw typePath.buildCodeFrameError('Propane map keys cannot be objects.');
    }
    if (typePath.isTSArrayType()) {
        throw typePath.buildCodeFrameError('Propane map keys cannot be arrays.');
    }
    if (typePath.isTSTypeReference() && isDateReference(typePath.node)) {
        throw typePath.buildCodeFrameError('Propane map keys cannot be Date objects.');
    }
    if (typePath.isTSTypeReference() && isUrlReference(typePath.node)) {
        throw typePath.buildCodeFrameError('Propane map keys cannot be URL objects.');
    }
    assertSupportedType(typePath, new Set()); // key types must still be valid primitives/identifiers; declared set unused here
}
export function assertSupportedTopLevelType(typePath) {
    if (isPrimitiveLikeType(typePath)) {
        return;
    }
    throw typePath.buildCodeFrameError('Propane files must export an object type or a primitive-like alias (string, number, boolean, bigint, null, undefined, Date, URL, ArrayBuffer, Brand).');
}
export function assertSupportedType(typePath, declaredTypeNames) {
    if (!typePath || !typePath.node) {
    ...
    if (typePath.isTSTypeReference()) {
        ...
        throw typePath.buildCodeFrameError('Propane property references must refer to imported or locally declared identifiers.');
    }
    if (typePath.isTSParenthesizedType()) {
    ...
    if (typePath.isTSTypeLiteral()) {
        for (const memberPath of typePath.get('members')) {
            if (!memberPath.isTSPropertySignature()) {
                throw memberPath.buildCodeFrameError('Propane nested object types can only contain property signatures.');
            }
            const keyPath = memberPath.get('key');
            if (!keyPath.isIdentifier() || memberPath.node.computed) {
                throw memberPath.buildCodeFrameError('Propane nested object properties must use simple identifier names.');
            }
            const nestedTypeAnnotation = memberPath.get('typeAnnotation');
            if (!nestedTypeAnnotation || !nestedTypeAnnotation.node) {
                throw memberPath.buildCodeFrameError('Propane nested object properties must include type annotations.');
            }
            assertSupportedType(nestedTypeAnnotation.get('typeAnnotation'), declaredTypeNames);
        }
        return;
    }
    throw typePath.buildCodeFrameError('Unsupported type in propane file. Only primitives, identifiers, Date, Brand, or object literals are allowed.');
}
export function isAllowedTypeReference(typePath, declaredTypeNames) {
...
export function registerTypeAlias(typeAlias, declaredTypeNames) {
...
function isPrimitiveLikeType(typePath) {
    ...
    if (typePath.isTSTypeReference()) {
        return (isDateReference(typePath.node)
