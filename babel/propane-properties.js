...
import { getArrayElementType, getMapTypeArguments, getSetTypeArguments, isArrayBufferReference, isArrayTypeNode, isDateReference, isImmutableArrayBufferReference, isMapTypeNode, isSetTypeNode, isUrlReference, } from './propane-type-guards.js';
...
export function extractProperties(memberPaths, generatedTypes, parentName, state, declaredTypeNames, declaredMessageTypeNames, getMessageReferenceName, assertSupportedType) {
    ...
    for (const memberPath of memberPaths) {
        if (!memberPath.isTSPropertySignature()) {
            throw memberPath.buildCodeFrameError('Propane object types can only contain property signatures.');
        }
...
function handleImplicitTypes(propTypePath, propName, generatedTypes, parentName, declaredTypeNames, declaredMessageTypeNames) {
    ...
    if (isMapTypeNode(propTypePath.node)) {
        ...
        if (mapArgs && t.isTSTypeLiteral(mapArgs.valueType)) {
            ...
            const newTypeAlias = t.tsTypeAliasDeclaration(t.identifier(newItemName), null, t.cloneNode(mapArgs.valueType));
            generatedTypes.push(newTypeAlias);
            declaredTypeNames.add(newItemName);
            declaredMessageTypeNames.add(newItemName);
            const newRef = t.tsTypeReference(t.identifier(newItemName));
            propTypePath.replaceWith(t.tsTypeReference(propTypePath.node.typeName, t.tsTypeParameterInstantiation([t.cloneNode(mapArgs.keyType), newRef])));
        }
    }
}
...
export function wrapImmutableType(node) {
    ...
    if (t.isTSArrayType(node)) {
        return t.tsTypeReference(t.identifier('ImmutableArray'), t.tsTypeParameterInstantiation([wrapImmutableType(t.cloneNode(node.elementType))]));
    }
