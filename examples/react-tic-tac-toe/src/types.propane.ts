...
export class BoardState extends Message<BoardState.Data> {
  ...
  protected $fromEntries(entries: Record<string, unknown>): BoardState.Data {
    ...
    const cellsArrayValue = cellsValue === undefined || cellsValue === null ? new ImmutableArray() : cellsValue instanceof ImmutableArray ? cellsValue : new ImmutableArray(cellsValue);
  ...
  override *[GET_MESSAGE_CHILDREN]() {
    yield ["cells", this.#cells] as [string, any];
  }
...
export class GameState extends Message<GameState.Data> {
  ...
  protected $fromEntries(entries: Record<string, unknown>): GameState.Data {
    ...
    const historyArrayValue = historyValue === undefined || historyValue === null ? new ImmutableArray() : historyValue instanceof ImmutableArray ? historyValue : new ImmutableArray(historyValue);
  ...
  override *[GET_MESSAGE_CHILDREN]() {
    yield ["history", this.#history] as [string, any];
  }
