import { assert, assertThrows, isMapValue } from './assert.ts';
import { MapMapKey } from './map-map-key.propane.ts';
...
export default function runMapMapKeyTests() {
  ...
  assert(instance.nested.get(inner) === 'alpha', 'Map key (Map) should roundtrip through constructor.');

  const serialized = instance.serialize();
  const hydrated = MapMapKey.deserialize(serialized);
  const hydratedCereal = hydrated.cerealize();
  assert(isMapValue(hydratedCereal.nested), 'Nested should remain a map after deserialize.');
  const [hydratedKey] = [...hydratedCereal.nested.keys()];
  ...
  assert(hydratedCereal.nested.get(hydratedKey) === 'alpha', 'Nested map value should survive serialize/deserialize.');

  const optKey = new Map<string, number>([['k', 2]]);
  const withOptional = instance.setOptional(new Map([[optKey, 42]]));
  assert(withOptional.optional?.get(optKey) === 42, 'Optional map should accept map keys.');
  ...
  assert(dedupe.nested.get(keyA) === 'second', 'Latest value should win for structurally equal map keys.');
}
