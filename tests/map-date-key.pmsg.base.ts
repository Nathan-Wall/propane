...
import { Message, WITH_CHILD, GET_MESSAGE_CHILDREN, ImmutableMap, equals, isTaggedMessageData, parseCerealString, ensure, SKIP, ImmutableDate, ImmutableUrl } from "../runtime/index.js";
...
export class MapDateKey extends Message<MapDateKey.Data> {
  ...
  $fromEntries(entries: Record<string, unknown>, options?: {
    ...
    if (!((ImmutableMap.isInstance(dateValuesMapValueConverted) || dateValuesMapValueConverted as object instanceof Map) && [...(dateValuesMapValueConverted as ReadonlyMap<unknown, unknown>).entries()].every(([mapKey, mapValue]) => typeof mapValue === "number"))) throw new Error("Invalid value for property \"dateValues\".");
    ...
    if (!((ImmutableMap.isInstance(urlValuesMapValueConverted) || urlValuesMapValueConverted as object instanceof Map) && [...(urlValuesMapValueConverted as ReadonlyMap<unknown, unknown>).entries()].every(([mapKey, mapValue]) => typeof mapValue === "string"))) throw new Error("Invalid value for property \"urlValues\".");
    ...
    if (optionalDateMapMapValueConverted !== undefined && !((ImmutableMap.isInstance(optionalDateMapMapValueConverted) || optionalDateMapMapValueConverted as object instanceof Map) && [...(optionalDateMapMapValueConverted as ReadonlyMap<unknown, unknown>).entries()].every(([mapKey, mapValue]) => typeof mapValue === "boolean"))) throw new Error("Invalid value for property \"optionalDateMap\".");
