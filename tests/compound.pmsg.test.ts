...
export default function runCompoundTests() {
  ...
  assert(User.isInstance(compoundFromData.user), 'Compound ctor should hydrate user data.');
  ...
  assert(Compound.Inline.isInstance(compoundFromData.inline), 'Compound inline should be exposed as Compound.Inline.');
  ...
  assert(User.isInstance(compoundFromMessages.user), 'Compound should accept user instances.');
  ...
  assert(
    Compound.Inline.isInstance(updatedInline.inline),
    'Compound setInline result should expose inline message class via namespace.'
  );
