...
interface ParentChainEntry {
  parent: WeakRef<Message<DataObject> | ImmutableArray<unknown> | ImmutableMap<unknown, unknown> | ImmutableSet<unknown>>;
  key: string | number;
}
...
export abstract class Message<T extends DataObject> {
  ...
  public [WITH_CHILD](
    _key: string | number,
    _child: Message<DataObject> | ImmutableArray<unknown> | ImmutableMap<unknown, unknown> | ImmutableSet<unknown>
  ): this {
    throw new Error(`${this.#typeName}[WITH_CHILD] not implemented. Regenerate with updated Babel plugin.`);
  }
   ...
   * Generated by Babel plugin for each message type.
   */
  public *[GET_MESSAGE_CHILDREN](): Iterable<[string | number, Message<DataObject> | ImmutableArray<unknown> | ImmutableMap<unknown, unknown> | ImmutableSet<unknown>]> {
   ...
   */
  public [EQUALS_FROM_ROOT](root: Message<DataObject> | null, other: Message<DataObject>): boolean {
  ...
  public $setParentChain(
    key: symbol,
    parent: Message<DataObject> | ImmutableArray<unknown> | ImmutableMap<unknown, unknown> | ImmutableSet<unknown>,
  ...
  public [SET_UPDATE_LISTENER](
    ...
    for (const [childKey, child] of this[GET_MESSAGE_CHILDREN]()) {
      if (child instanceof Message) {
        ...
        child.$setParentChain(key, this as unknown as Message<DataObject>, childKey);
        // Recursively set up listener on child
        child[SET_UPDATE_LISTENER](key, callback);
      } else if (child && typeof child === 'object' && SET_UPDATE_LISTENER in child) {
        // Handle ImmutableArray/Map/Set
        const collection = child as {
          [SET_UPDATE_LISTENER]: (key: symbol, callback: UpdateListenerCallback, parent: Message<DataObject>, parentKey: string | number) => void;
        };
        collection[SET_UPDATE_LISTENER](key, callback, this as unknown as Message<DataObject>, childKey);
      }
    }
  }
   ...
   */
  public [PROPAGATE_UPDATE](key: symbol, replacement: Message<DataObject>): void {
    ...
    if (chain?.parent.deref()) {
      ...
      if (parent instanceof Message) {
        ...
        parent[PROPAGATE_UPDATE](key, newParent as Message<DataObject>);
      }
  ...
  protected $propagateUpdates(replacement: this): void {
    ...
    for (const key of this.#parentChains.keys()) {
      this[PROPAGATE_UPDATE](key, replacement as unknown as Message<DataObject>);
    }
